...
arch-chroot /mnt /bin/bash <<EOF
    ...
    # Ensure system time is accurate (GPG requirement)
    # Since timedatectl might not work in chroot, we try a basic sync or assume host time is correct
    ...
    # --- UPDATED CACHYOS INTEGRATION ---
    echo "Hardening CachyOS GPG retrieval..."
    # Attempting retrieval from Ubuntu keyserver first as it is more resilient to failure
    pacman-key --recv-keys F3B607488DB35A47 --keyserver keyserver.ubuntu.com || \
    pacman-key --recv-keys F3B607488DB35A47 --keyserver hkps://keys.openpgp.org || \
    { echo "CRITICAL: Keyserver failure. Manual intervention required."; exit 1; }
